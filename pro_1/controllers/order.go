...
func OrderShowByDate(c *gin.Context) {
	...
	if err := models.MPosGORM.Raw("SELECT * from calix_proj_schema.orders where phone = ? AND date(create_dtm) = to_date(?, 'DD-Mon-YYYY')", query.Phone, query.Date).Scan(&orders).Error; err != nil {
...
func OrderShowByPhone(c *gin.Context) {
	...
	if err := models.MPosGORM.Raw("SELECT * from calix_proj_schema.orders where phone = ?", phone).Scan(&orders).Error; err != nil {
...
func OrderShowByID(c *gin.Context) {
	...
	if err := models.MPosGORM.Raw("SELECT * from calix_proj_schema.orders where order_id = ?", order_id).Scan(&orders).Error; err != nil {
